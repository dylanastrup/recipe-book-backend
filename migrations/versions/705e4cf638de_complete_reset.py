...
def upgrade():
    # --- 1. TABLES WITH NO EXTERNAL DEPENDENCIES (Users, Tags, etc.) ---
    ...
    # --- 2. TABLES DEPENDENT ON USERS (Feedback) ---
    ...
    # --- 3. TABLES DEPENDENT ON USERS & SELF (Recipes) ---
    ...
    # NOTE: The self-referencing FK to 'recipes' (original_recipe_id) might still cause issues
    # but we proceed with this order as it's the general requirement.
    sa.ForeignKeyConstraint(['original_recipe_id'], ['recipes.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    
    # --- 4. TABLES DEPENDENT ON RECIPES / FEEDBACK (Images, Ratings, Upvotes, etc.) ---
    op.create_table('feedback_upvotes',
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.Column('feedback_id', sa.Integer(), nullable=False),
    # REFERENCES now work because feedback and users are defined
...
def downgrade():
    # ... (Your downgrade function remains correct for this order)
